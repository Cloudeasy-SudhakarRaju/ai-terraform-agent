...
async def chat(req: Request):
    ...
    if any(kw in user_input for kw in ["create ec2", "launch instance", "spin up vm"]):
        ...
        status, result = await trigger_pipeline(region)
...
# === Trigger Azure Pipeline (with region variable) ===
async def trigger_pipeline(region: str):
    pipeline_id = fetch_pipeline_id(AZURE_ORG, AZURE_PROJECT, AZURE_PIPELINE_NAME, AZURE_PAT)
    if not pipeline_id:
        return 404, f"Pipeline '{AZURE_PIPELINE_NAME}' not found in project."
