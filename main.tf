...
from openai import OpenAI  # ✅ Use the new OpenAI class (>=1.0.0)

# Load environment variables
load_dotenv()

# Configure Together.ai using OpenAI-compatible SDK
client = OpenAI(
...
def together_ai_response(message: str) -> str:
    try:
        response = client.chat.completions.create(
            model="mistralai/Mistral-7B-Instruct-v0.1",  # You can also try "meta-llama/Llama-3-8B-Instruct"
            messages=[
